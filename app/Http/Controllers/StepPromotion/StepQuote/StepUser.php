...
{
    public function requestQuoteTables(Request $request){
        try {
            $user=$request->user();
        ...
        if($count==0){
            ...
            try {
                $current = Carbon::now();
                $Quote=package_file::where('user_id',$user->id)->update(['Quote'=>2,'QuoteReq'=>$current]);
                if($Quote){
                    $quoteMail=User::select('email','name')->where('Leader',1)->first();
                    try {
                        Mail::raw('لديكم طلب استلال الكتروني من قبل : '.$user->name, function ($message) use($quoteMail,) {
                            $message->from(env('MAIL_FROM_ADDRESS'), env('MAIL_FROM_NAME'));
                            $message->to($quoteMail->email, $quoteMail->name)->subject('الى رئيس القسم المحترم يرجى التفضل بالموافقة على ارسال البحوث الى لجنة الاستلال الالكتروني');
                            });
                    } catch (\Throwable $th) {
